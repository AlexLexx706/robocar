...
class FFmpegReader:
    ...
    def process_direct_show_video(self, video_device_name="USB2.0 PC CAMERA", size=None):
        ...
        params = u"{path} -f dshow -i video=\"{video_device_name}\" -f rawvideo -pix_fmt bgr24 -s {width}x{height} -".format(
    ...
    def process_net_stream(self, port, size=None):
        if size is not None:
            self.size = size
            self.buffer_size = self.size[0] * self.size[1] * 3
        nc_params = u"{path} -u -l -p {port}".format(path="nc", port=port)
        logger.info(nc_params)
        self.nc_proc = subprocess.Popen(nc_params.split(), stdout=subprocess.PIPE)

        if self.save_dir is None:
            params = u"{path} -y -f h264 -i - -f rawvideo -pix_fmt bgr24 -s {width}x{height} -".format(port = port,
                     path="ffmpeg", width = self.size[0], height = self.size[1])
        else:
        ...
        logger.info(params)
        self.proc = subprocess.Popen(params.split(), stdin=self.nc_proc.stdout, stdout=subprocess.PIPE)
