...
class Client:
    ...
    def send_cmd(self, cmd, *data):
        buffer = json.dumps((cmd, data)) if self.serialization == "json" else msgpack.packb((cmd, data), use_bin_type=True)
        buffer = struct.pack("<L", len(buffer)) + buffer
        self.sock.sendall(buffer)
        size = struct.unpack("<L", self.read(4))[0]
        res = json.loads(self.read(size)) if self.serialization == "json" else msgpack.unpackb(self.read(size), encoding='utf-8')
